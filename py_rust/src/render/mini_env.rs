...
pub fn new_mini_env<'a>(root: &Path, state: &'a State) -> Result<minijinja::Environment<'a>, Zerr> {
    ...
    env.set_syntax(minijinja::Syntax {
        block_start: state.conf.engine.block_start.clone().into(),
        block_end: state.conf.engine.block_end.clone().into(),
        variable_start: state.conf.engine.variable_start.clone().into(),
        variable_end: state.conf.engine.variable_end.clone().into(),
        comment_start: state.conf.engine.comment_start.clone().into(),
        comment_end: state.conf.engine.comment_end.clone().into(),
    })
